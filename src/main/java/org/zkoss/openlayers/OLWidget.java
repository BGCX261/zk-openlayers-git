...
public abstract class OLWidget implements JSONAware {
    ...
    protected void clientUpdate(String attr, Object... args) {
        if (_map != null)
            ...
            switch (args.length) {
            case 0:
                getNativeObject().invoke(attr);//添加到动作队列，稍后客户端执行
                break;
            case 1:
            ...
            case 3:
                ...
                break;
            case 4:
                getNativeObject().invoke(attr, args[0], args[1], args[2], args[3]);
                break;
            case 5:
                getNativeObject().invoke(attr, args[0], args[1], args[2], args[3], args[4]);
                break;
            case 6:
                getNativeObject().invoke(attr, args[0], args[1], args[2], args[3], args[4], args[5]);
                break;
            case 7:
                getNativeObject().invoke(attr, args[0], args[1], args[2], args[3], args[4], args[5], args[6]);
                break;
            case 8:
                getNativeObject().invoke(attr, args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7]);
                break;
            }
        }
    }
    ...
    public String getUuid() {
        if (_uuid == null) {
            _uuid = _map != null ? ((DesktopCtrl) _map.getDesktop()).getNextUuid(_map) : getNextUuid();
        }
        return _uuid;
    }
    ...
    protected static String getNextUuid() {
        ...
        String uuid = null;
        try {
            uuid = ANONYMOUS_ID + (_prefix != null ? String.valueOf(_prefix) : "") + _anonymousId++;
            if (_anonymousId == Integer.MAX_VALUE) {
                ...
                if (_prefix == null)
                    ...
                    if (c == _chars.get(_chars.size() - 1).charAt(0)) {
                        ...
                        _prefix[len - 1] = _chars.get(_chars.indexOf(Character.toString(_prefix[len - 1])) + 1).charAt(0);
                }
            }
        } finally {
            _lock.unlock();
    ...
    public Function toClientWidget() {
        if (_map == null)
            throw new UnsupportedOperationException("The OLWidget is not attached to Openlayers!");
        StringBuilder sb = new StringBuilder();
        return new ReturnFunction(sb.append("(function () {return openlayers._binds['")//
                .append(_map.getUuid()).append("']['")//
                .append(getUuid()).append("'];})()").toString());
